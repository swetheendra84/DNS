...
void spoof(struct info *arg,const struct pcap_pkthdr *header,const u_char *packet)
{
	 ...
	 /* get cleaned up IPs */
        ...
        /* get spoof IP */
        if (!strcmp(arg->domain, "spoof_all")) {
                spoof_it = 1;
        ...
        if (spoof_it == 1) {
		 ...
		 dns_question_in = (struct query*)(((char*) dns_hdr) + sizeof(struct dns_header));
...
if(flag!=0)
{
	...
	while(getline(&line,&len,fp)!=-1)
	{
		char *token=strtok(line,"\t\n");
			...
			in->sip=token;
			//memcpy(in->sip,token,16);
			token=strtok(NULL," \t\n");
	                //memcpy(in->domain,token,strlen(token));
			in->domain=token;
	                in->next=NULL;
